...
func ArticleDisplayContentByHash(db *sql.DB, rawhash string) {

	hash, err := strconv.ParseUint(rawhash, 10, 32)
	if err != nil {
		log.Fatal("Bad Number:", rawhash)
	}

	row, err := db.Query("SELECT id,title,url,keywords,description,content FROM articles where hash=?", hash)
	if err != nil {
		log.Fatal(err)
	}
	defer row.Close()
...
func displayArticlByDay(db *sql.DB, days int) {
	...
	myname := os.Getenv("FILEREQUESTED")
